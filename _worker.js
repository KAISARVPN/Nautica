...
<html lang="en" id="html" class="scroll-auto scrollbar-hide dark">
  ...
  <body class="bg-white dark:bg-neutral-800 bg-fixed">
    ...
    <div class="container">
      <div
        id="container-title"
        class="sticky top-0 bg-white/80 dark:bg-neutral-800/80 backdrop-blur-md border-b border-neutral-200 dark:border-neutral-700 shadow-sm z-10 py-6 w-screen"
      >
...
class Document {
  ...
  buildProxyGroup() {
    ...
    for (let i = 0; i < this.proxies.length; i++) {
      ...
      // Assign proxies
      // Tambahkan shadow, border lebih tipis, dan efek hover
      proxyGroupElement += `<div class="lozad scale-95 mb-2 bg-white dark:bg-neutral-900 transition-all duration-300 rounded-xl p-4 w-60 border border-neutral-200 dark:border-neutral-700 shadow-md hover:shadow-xl hover:scale-105">`;
      // Ganti border di ikon bendera
      proxyGroupElement += `  <div id="countryFlag" class="absolute -translate-y-9 -translate-x-2 border-2 border-white dark:border-neutral-900 rounded-full overflow-hidden"><img width="32" src="https://hatscripts.github.io/circle-flags/flags/${proxyData.country.toLowerCase()}.svg" /></div>`;
      proxyGroupElement += `  <div>`;
      proxyGroupElement += `    <div id="ping-${i}" class="animate-pulse text-xs font-semibold dark:text-neutral-300">Idle ${proxyData.proxyIP}:${proxyData.proxyPort}</div>`;
      proxyGroupElement += `  </div>`;
      // Ganti header amber dengan gradient biru yang modern dan teks putih
      proxyGroupElement += `  <div class="rounded py-1 px-2 bg-gradient-to-r from-blue-500 to-indigo-600">`;
      proxyGroupElement += `    <h5 class="font-bold text-md text-white mb-1 overflow-x-scroll scrollbar-hide text-nowrap">${proxyData.org}</h5>`;
      ...
      for (let x = 0; x < proxyData.list.length; x++) {
        ...
        // Tambahkan transisi, style dark mode yang konsisten, dan efek hover
        proxyGroupElement += `<button class="bg-blue-600 dark:bg-blue-700 text-white text-xs rounded-md p-1.5 w-full transition-all duration-200 ease-in-out transform hover:bg-blue-700 dark:hover:bg-blue-800 hover:scale-105" onclick="copyToClipboard('${proxy}')">${indexName[x]}</button>`;
