...
function getAllConfig(request, hostName, proxyList, page = 0) {
  ...
  try {
    ...
    // PERUBAHAN: Judul diubah ke tema horor
    document.setTitle("Welcome to the <span class='font-display text-3xl text-red-600 animate-pulse'>Haunted Store</span>");
 ...
 *
 * PERUBAHAN v2 (HORROR THEME):
 * - Palet warna diubah dari Biru/Abu-abu menjadi Merah/Hitam/Abu-abu gelap.
 * - Font judul diubah menjadi 'Creepster' dari Google Fonts untuk nuansa horor.
 * - Teks dan judul diubah agar sesuai dengan tema (cth: "Haunted Store", "Cursed Servers").
 * - Animasi AOS diubah sedikit (durasi lebih lambat, easing berbeda).
 * - Notifikasi diubah menjadi merah dengan teks "Terciduk! Konfigurasi telah dicuri!".
 * - Tombol-tombol diubah menjadi merah.
 * - Latar belakang default adalah mode gelap.
 */
let baseHTML = `
<!DOCTYPE html>
<html lang="en" id="html" class="scroll-smooth dark">
  <head>
    ...
    <title>Haunted Proxy Store</title>
        ...
        theme: {
          extend: {
            fontFamily: {
              ...
              // PERUBAHAN: Mendefinisikan font 'font-display' kustom
  ...
  <body class="bg-neutral-900 dark:bg-black text-neutral-200 dark:text-neutral-200 selection:bg-red-700 selection:text-white">

    <aside data-aos="fade-right" class="fixed top-0 left-0 z-40 w-20 h-screen bg-neutral-800 dark:bg-neutral-900 shadow-lg shadow-red-900/50 transition-all duration-300">
      <div class="flex flex-col items-center h-full py-4 space-y-4 overflow-y-auto scrollbar-hide">
        <a href="/sub" class="p-2 rounded-lg hover:bg-neutral-700 dark:hover:bg-neutral-700" title="Home">
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="size-7 text-red-600 hover:animate-pulse">
            <path d="M11.47 3.841a.75.75 0 0 1 1.06 0l8.69 8.69a.75.75 0 1 0 1.06-1.061l-8.689-8.69a2.25 2.25 0 0 0-3.182 0l-8.69 8.69a.75.75 0 1 0 1.061 1.06l8.69-8.689Z" />
            <path d="m12 5.432 8.159 8.159c.03.03.06.058.091.086v6.198c0 1.035-.84 1.875-1.875 1.875H15a.75.75 0 0 1-.75-.75v-4.5a.75.75 0 0 0-.75-.75h-3a.75.75 0 0 0-.75.75V21a.75.75 0 0 1-.75.75H5.625a1.875 1.875 0 0 1-1.875-1.875v-6.198a2.29 2.29 0 0 0 .091-.086L12 5.432Z" />
          </svg>
        </a>
        <hr class="w-3/4 border-neutral-700 dark:border-neutral-700">
    ...
    <main class="ml-20 transition-all duration-300">
      ...
      <header data-aos="fade-down" class="sticky top-0 z-30 bg-neutral-800/80 dark:bg-neutral-900/80 backdrop-blur-md shadow-lg shadow-red-900/30">
        <div class="container mx-auto px-6 py-4">
          <div class="flex items-center justify-between">
            <h1 id="container-title" class="text-2xl font-bold text-white dark:text-white">
              PLACEHOLDER_JUDUL
            </h1>
            
            <div class="flex items-center gap-6">
              <div id="container-info" class="flex items-center gap-4 text-xs text-neutral-400 dark:text-neutral-400">
              ...
              <button onclick="toggleDarkMode()" class="bg-neutral-700 dark:bg-neutral-700 rounded-full p-2 transition-all hover:bg-neutral-600 dark:hover:bg-neutral-600" title="Toggle Dark Mode">
      ...
      <section class="container mx-auto px-6 py-8" data-aos="fade-up" data-aos-delay="100">
        <div class="p-8 bg-neutral-800 dark:bg-neutral-800 rounded-lg shadow-md shadow-red-900/50">
          <h2 class="text-4xl font-display text-red-500 mb-3">Explore Our Cursed Servers</h2>
          <p class="text-neutral-400 dark:text-neutral-400 mb-2">Choose your connection from the shadows... Fast, secure, and terrifyingly reliable.</p>
          <div class="flex gap-4 text-sm font-medium text-neutral-200 dark:text-neutral-200">
            PLACEHOLDER_INFO
          </div>
        </div>
      </section>
    ...
    <footer class="fixed bottom-6 right-6 z-40 flex flex-col gap-3">
      <a href="${DONATE_LINK}" target="_blank" title="Donasi">
        <button class="bg-red-700 text-white rounded-full p-3 shadow-lg transition-all hover:bg-red-800 hover:scale-110">
      ...
      <button onclick="toggleWildcardsWindow()" title="Wildcard Domains" class="bg-red-700 text-white rounded-full p-3 shadow-lg transition-all hover:bg-red-800 hover:scale-110 PLACEHOLDER_API_READY">
    ...
    <div
      ...
      class="fixed z-50 opacity-0 transition-all ease-in-out duration-300 top-24 right-6 p-4 max-w-sm bg-neutral-800 dark:bg-neutral-800 rounded-xl shadow-lg border border-neutral-700 dark:border-neutral-700 flex items-center gap-x-4"
    >
      <div class="shrink-0">
      ...
      <div>
        <div class="text-md font-bold text-red-600">Terciduk!</div>
        <p class="text-sm text-neutral-200 dark:text-neutral-200">Konfigurasi telah dicuri!</p>
      </div>
    </div>

    <div id="container-window" class="hidden fixed inset-0 z-40 bg-black/80 backdrop-blur-sm">
      
      <div data-aos="zoom-in" data-aos-duration="300" class="fixed z-50 top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[90%] max-w-sm p-4">
        ...
        <div id="output-window" class="w-full h-full flex flex-col gap-4 text-center rounded-md hidden">
          ...
          <div class="flex flex-wrap justify-center gap-2">
            <button onclick="copyToClipboardAsTarget('clash')" class="flex-grow basis-28 p-3 rounded-lg bg-red-700 text-white font-semibold transition-all hover:bg-red-800 hover:scale-105">
              Clash
            </button>
            <button onclick="copyToClipboardAsTarget('sfa')" class="flex-grow basis-28 p-3 rounded-lg bg-red-700 text-white font-semibold transition-all hover:bg-red-800 hover:scale-105">
              SFA
            </button>
            <button onclick="copyToClipboardAsTarget('bfr')" class="flex-grow basis-28 p-3 rounded-lg bg-red-700 text-white font-semibold transition-all hover:bg-red-800 hover:scale-105">
              BFR
            </button>
            <button onclick="copyToClipboardAsTarget('v2ray')" class="flex-grow basis-40 p-3 rounded-lg bg-neutral-700 dark:bg-neutral-700 text-white dark:text-white font-semibold transition-all hover:bg-neutral-600 dark:hover:bg-neutral-600 hover:scale-105">
              V2Ray/Xray
            </button>
            <button onclick="copyToClipboardAsRaw()" class="flex-grow basis-40 p-3 rounded-lg bg-neutral-700 dark:bg-neutral-700 text-white dark:text-white font-semibold transition-all hover:bg-neutral-600 dark:hover:bg-neutral-600 hover:scale-105">
              Raw
            </button>
          </div>
          <div class="mt-4">
            <button onclick="toggleOutputWindow()" class="w-full p-3 border-2 border-neutral-600 text-neutral-400 font-semibold rounded-lg transition-all hover:bg-neutral-600 hover:text-white">
              Close
            </button>
          </div>
          </div>
        ...
        <div id="wildcards-window" class="w-full h-full flex flex-col gap-4 rounded-md hidden">
          <div class="flex w-full h-full gap-2 justify-between">
              <input
                ...
                class="flex-grow w-full h-full px-4 py-3 rounded-lg focus:outline-0 focus:ring-2 focus:ring-red-600 border-none bg-neutral-700 text-white"
              />
              <button
                onclick="registerDomain()"
                class="p-3 rounded-lg bg-red-700 text-white transition-all hover:bg-red-800"
              >
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="size-6">
          ...
          <div class="basis-5/6 w-full h-80 rounded-md bg-neutral-800 dark:bg-neutral-800 p-4">
            <div
              id="container-domains"
              class="w-full h-full rounded-md flex flex-col gap-2 overflow-y-scroll scrollbar-hide text-white dark:text-white"
            >
              </div>
          </div>
          <button
                onclick="toggleWildcardsWindow()"
                class="w-full border-2 border-neutral-600 text-neutral-400 hover:bg-neutral-600 dark:text-white p-3 rounded-lg flex justify-center items-center transition-all"
              >
                Close
              </button>
        </div>
    ...
    <script>
        ...
        const res = fetch(url).then(async (res) => {
          ...
          if (res.status == 200) {
            ...
            for (const domain of respJson) {
              ...
              domainElement.classList.add("w-full", "bg-neutral-700", "dark:bg-neutral-700", "p-3", "rounded-md", "font-medium");
      ...
      // PERUBAHAN: Fungsi untuk set ikon dark mode dipisah
      ...
      function toggleDarkMode() {
        ...
        setDarkModeIcon(); // Panggil fungsi set ikon
      }
              ...
              .then(async (res) => {
                ...
                if (res.status == 200) {
                  ...
                  if (jsonResp.proxyip === true) {
                    ...
                    // PERUBAHAN: Diubah menjadi hijau neon (toxic green)
                    pingElement.classList.add("text-green-400");
                  } else {
      ...
      function checkRegion() {
        ...
        for (let i = 0; ; i++) {
          ...
          const res = fetch(
            ...
            if (res.status == 200) {
              containerRegionCheck.innerHTML = "<hr class='my-1 border-neutral-600 dark:border-neutral-600'>";
      ...
      window.onload = () => {
        ...
        // PERUBAHAN: Durasi dan easing diubah untuk nuansa lebih lambat/horor
        AOS.init({
          duration: 800,
        ...
        // PERUBAHAN: Memanggil setDarkModeIcon alih-alih toggle ganda
        setDarkModeIcon(); 
...
class Document {
  ...
  addInfo(text) {
    // PERUBAHAN: Info box diubah ke tema gelap
    text = `<span class="bg-neutral-700 dark:bg-neutral-700 px-3 py-1 rounded-full">${text}</span>`;
  ...
  buildProxyGroup() {
    ...
    if (this.proxies.length === 0) {
        // PERUBAHAN: Tampilan "tidak ditemukan" disesuaikan dengan tema gelap
        proxyGroupElement = `
        <div class="col-span-full text-center py-10" data-aos="fade-up">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-16 mx-auto text-neutral-600">
              <path stroke-linecap="round" stroke-linejoin="round" d="m21 21-5.197-5.197m0 0A7.5 7.5 0 1 0 5.196 5.196a7.5 7.5 0 0 0 10.607 10.607Z" />
            </svg>
            <h3 class="mt-4 text-2xl font-semibold text-neutral-300 dark:text-neutral-300">Tidak Ada Server Ditemukan</h3>
        ...
        for (let i = 0; i < this.proxies.length; i++) {
          ...
          // PERUBAHAN: Ini adalah "Product Card" yang baru dengan tema horor
          proxyGroupElement += `
          <div data-aos="fade-up" data-aos-delay="${(i % 4) * 100}" class="lozad bg-neutral-800 dark:bg-neutral-800 rounded-lg shadow-lg shadow-red-900/30 overflow-hidden transition-all duration-300 hover:shadow-2xl hover:shadow-red-700/50 hover:-translate-y-1">
            <div class="p-4 bg-neutral-700/50 dark:bg-neutral-900/50 border-b border-neutral-700 dark:border-neutral-700">
              <div class="flex items-center justify-between">
                <div class="flex items-center gap-3">
                  ...
                  <h5 class="font-bold text-lg text-red-500 dark:text-red-500 truncate">${proxyData.org}</h5>
                </div>
                <div id="ping-${proxyIndex}" data-target="${proxyData.proxyIP}:${proxyData.proxyPort}" class="text-xs font-semibold text-neutral-400 dark:text-neutral-400">Idle</div>
              </div>
            </div>

            <div class="p-4">
              <div class="text-sm text-neutral-300 dark:text-neutral-300 mb-4">
                <p><span class="font-medium text-neutral-100 dark:text-neutral-100">IP:</span> ${proxyData.proxyIP}</p>
                <p><span class="font-medium text-neutral-100 dark:text-neutral-100">Port:</span> ${proxyData.proxyPort}</p>
  ...
  buildProxyButtons(proxyList, proxyIndex) {
    ...
    for (let x = 0; x < proxyList.length; x++) {
      ...
      // PERUBAHAN: Tombol diubah menjadi merah
      buttonsHTML += `<button class="bg-red-700 text-white text-xs font-semibold rounded-md p-2 w-full transition-all duration-200 ease-in-out transform hover:bg-red-800 hover:scale-105" onclick="copyToClipboard('${proxy}')">${indexName[x]}</button>`;
    }
    return buttonsHTML;
  }
  ...
  buildCountryFlag() {
    ...
    for (const flag of new Set(flagList)) {
      ...
      // PERUBAHAN: Tombol bendera disesuaikan dengan tema gelap
      flagElement += `
      <a href="/sub?${queryParams.toString()}" class="p-2 rounded-lg transition-all hover:bg-neutral-700 dark:hover:bg-neutral-700" title="${flag}">
  ...
  addPageButton(text, link, isDisabled) {
    // PERUBAHAN: Tombol paginasi diubah ke tema gelap dengan hover merah
    const pageButton = `
    <li>
      <button ${isDisabled ? "disabled" : ""} 
              class="px-5 py-2 font-semibold bg-neutral-800 text-white dark:bg-neutral-800 border border-neutral-700 dark:border-neutral-700 rounded-lg shadow-sm transition-all hover:bg-red-700 dark:hover:bg-red-800 hover:text-white hover:border-red-700 disabled:opacity-50 disabled:cursor-not-allowed" 
