...
<html lang="en" id="html" class="scroll-auto scrollbar-hide dark">
  ...
  <body class="bg-white dark:bg-neutral-900 bg-fixed">
    <div
    ...
    <div class="container">
      <div
        id="container-title"
        class="sticky top-0 bg-white/80 dark:bg-neutral-900/80 backdrop-blur-md border-b border-neutral-200 dark:border-neutral-700 shadow-sm z-10 py-6 w-screen"
      >
      ...
      <div class="w-full px-4 pt-4 overflow-x-auto scrollbar-hide">
        <div class="flex justify-center space-x-3 pb-2">
          PLACEHOLDER_BENDERA_NEGARA
        </div>
      </div>

      <div class="flex gap-6 pt-4 w-screen justify-center">
    ...
    <script>
      ...
      function checkProxy() {
        for (let i = 0; ; i++) {
          ...
          // #### PERUBAHAN DI SINI ####
          // Mengambil target IP:PORT dari data-target, bukan textContent
              ...
              .then(async (res) => {
                ...
                if (res.status == 200) {
                  ...
                  if (jsonResp.proxyip === true) {
                    isActive = true;
                    // Tampilan ping baru
                    pingElement.textContent = jsonResp.delay + " ms";
                    pingElement.classList.remove("animate-pulse", "text-yellow-400");
                    pingElement.classList.add("text-yellow-500", "dark:text-yellow-400");
                    
                    // Menambahkan badge "ACTIVE" yang sebelumnya tersembunyi
                    const activeBadge = document.getElementById("active-badge-" + i);
...
class Document {
  ...
  buildProxyGroup() {
    ...
    // #### PERUBAHAN DISINI: Menggunakan grid yang lebih responsif dan padding (px-4) ####
    proxyGroupElement += `<div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 px-4">`;
    for (let i = 0; i < this.proxies.length; i++) {
      const proxyData = this.proxies[i];

      // #### PERUBAHAN TOTAL PADA KARTU ####

      // Kartu: Mengganti bg-neutral-900 menjadi bg-neutral-800 agar sedikit lebih terang dari body (bg-neutral-900)
      proxyGroupElement += `<div class="lozad scale-95 mb-2 bg-white dark:bg-neutral-800 transition-all duration-300 rounded-xl p-4 shadow-md hover:shadow-xl hover:scale-105">`;

      // Bagian Atas: Status, Ping, Bendera, Kode Negara
      proxyGroupElement += `  <div class="flex justify-between items-center mb-3">`;
      // Status & Ping
      proxyGroupElement += `    <div class="flex items-center gap-2">`;
      // Badge "ACTIVE" (awalnya disembunyikan, muncul setelah checkProxy sukses)
      proxyGroupElement += `      <span id="active-badge-${i}" class="hidden flex items-center gap-1 text-xs font-semibold text-green-500">`;
      proxyGroupElement += `        <svg class="w-3 h-3" fill="currentColor" viewBox="0 0 8 8"><circle cx="4" cy="4" r="4"/></svg>`;
      proxyGroupElement += `        ACTIVE`;
      proxyGroupElement += `      </span>`;
      // Ping: Menambahkan data-target untuk JS
      proxyGroupElement += `      <span id="ping-${i}" data-target="${proxyData.proxyIP}:${proxyData.proxyPort}" class="animate-pulse text-xs font-semibold text-yellow-500 dark:text-yellow-400">Checking...</span>`;
      proxyGroupElement += `    </div>`;
      // Bendera & Kode Negara
      proxyGroupElement += `    <div class="flex items-center gap-2">`;
      proxyGroupElement += `      <span class="text-xs font-bold text-gray-500 dark:text-gray-400">${proxyData.country}</span>`;
      proxyGroupElement += `      <img width="24" height="24" class="rounded-full" src="https://hatscripts.github.io/circle-flags/flags/${proxyData.country.toLowerCase()}.svg" />`;
      proxyGroupElement += `    </div>`;
      proxyGroupElement += `  </div>`;

      // Bagian Tengah: Nama Organizasi & IP
      proxyGroupElement += `  <div class="mb-4">`;
      proxyGroupElement += `    <h5 class="font-bold text-md text-gray-900 dark:text-white mb-1 overflow-x-scroll scrollbar-hide text-nowrap">${proxyData.org}</h5>`;
      proxyGroupElement += `    <p class="text-sm text-gray-600 dark:text-gray-400">${proxyData.proxyIP}:${proxyData.proxyPort}</p>`;
      proxyGroupElement += `  </div>`;

      // Input tersembunyi untuk checkRegion (jika masih diperlukan)
      proxyGroupElement += `  <div id="container-region-check-${i}" class="hidden">`;
      proxyGroupElement += `    <input id="config-sample-${i}" class="hidden" type="text" value="${proxyData.list[0]}">`;
      proxyGroupElement += `  </div>`;

      // Bagian Bawah: Tombol (Grid 3x2)
      proxyGroupElement += `  <div class="grid grid-cols-3 gap-2 text-sm">`;
      const indexName = [
        `${reverse("NAJORT")} TLS`,
        `${reverse("SSELV")} TLS`,
        `${reverse("SS")} TLS`,
      ...
      for (let x = 0; x < proxyData.list.length; x++) {
        ...
        // Tombol baru yang lebih sesuai dengan gambar
        proxyGroupElement += `<button class="bg-blue-600 dark:bg-blue-700 text-white text-xs font-semibold rounded-md p-2 w-full transition-all duration-200 ease-in-out transform hover:bg-blue-700 dark:hover:bg-blue-800 hover:scale-105" onclick="copyToClipboard('${proxy}')">${indexName[x]}</button>`;
      }
      proxyGroupElement += `  </div>`; // Penutup grid tombol
      proxyGroupElement += `</div>`; // Penutup kartu
    }
    proxyGroupElement += `</div>`; // Penutup grid utama
  ...
  buildCountryFlag() {
    ...
    for (const flag of new Set(flagList)) {
      // #### PERUBAHAN DISINI: Menyesuaikan style bendera untuk navbar horizontal ####
      flagElement += `<a href="/sub?cc=${flag}${
        proxyBankUrl ? "&proxy-list=" + proxyBankUrl : ""
      }" class="flex-shrink-0 p-1" ><img width="32" height="32" class="rounded-full transition-all duration-200 hover:scale-110 hover:shadow-lg" src="https://hatscripts.github.io/circle-flags/flags/${flag.toLowerCase()}.svg" /></a>`;
    }
