...
function getAllConfig(request, hostName, proxyList, page = 0) {
  ...
  try {
    ...
    document.setTitle("Welcome to the <span class='text-red-500 font-horror'>KaisarStore</span>"); // Diubah
    document.addInfo(`Total Items: ${proxyList.length}`);
...
<html lang="en" id="html" class="scroll-auto scrollbar-hide dark">
  <head>
    ...
    <title>KaisarStore // Proxy Emporium</title>
    <script src="https://cdn.tailwindcss.com"></script>
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Creepster&family=EB+Garamond:ital,wght@0,400;0,700;1,400&display=swap" rel="stylesheet">
    <style>
      ...
      /* Animasi Flicker untuk Judul */
      @keyframes flicker {
        0%, 100% { opacity: 1; text-shadow: 0 0 8px #ef4444, 0 0 12px #ef4444; }
        50% { opacity: 0.7; text-shadow: 0 0 8px #ef4444, 0 0 10px #ef4444; }
      }
      .title-flicker {
        animation: flicker 3s infinite;
      }
      
      /* Animasi Denyut Halus untuk Kartu */
      @keyframes subtle-pulse {
        0%, 100% { transform: scale(1); box-shadow: 0 0 15px rgba(239, 68, 68, 0.1); }
        50% { transform: scale(1.02); box-shadow: 0 0 30px rgba(239, 68, 68, 0.3); }
      }
      .card-pulse {
        animation: subtle-pulse 6s infinite ease-in-out;
      }

      /* Animasi Static Noise Overlay */
      ...
      body.dark::before {
        ...
        width: 100vw;
        height: 100vh;
        background: url('data:image/svg+xml;base64,');
  ...
  <body class="bg-neutral-900 dark:bg-black font-serif text-gray-300 bg-fixed">
    <div
      id="notification-badge"
      class="fixed z-50 opacity-0 transition-opacity ease-in-out duration-300 mt-9 mr-6 right-0 p-3 max-w-sm bg-neutral-800 rounded-xl border border-2 border-red-500 flex items-center gap-x-4 shadow-lg shadow-red-500/20"
    >
      <div class="shrink-0">
      ...
      <div>
        <div class="text-lg font-bold font-horror text-red-500">Berhasil!</div>
        <p class="text-sm text-gray-300">Akun berhasil disalin</p>
      </div>
    </div>
    <div>
      <div
        class="h-full fixed top-0 w-14 bg-neutral-900 dark:bg-black border-r-2 border-red-900/50 z-20 overflow-y-scroll scrollbar-hide"
      >
    ...
    <div id="container-header">
      <div id="container-info" class="bg-red-900/30 border-b-2 border-red-700/50 text-right px-5 py-1">
        <div class="flex justify-end gap-3 text-sm text-gray-400">
    ...
    <div class="container">
      <div
        id="container-title"
        class="sticky top-0 bg-black/80 backdrop-blur-md border-b border-red-700/50 shadow-lg shadow-red-900/30 z-10 py-6 w-screen"
      >
        <h1 class="text-5xl text-center text-red-500 font-horror title-flicker">
          PLACEHOLDER_JUDUL
        </h1>
      </div>
    ...
    <div id="container-window" class="hidden">
      <div class="fixed z-20 top-0 w-full h-full bg-black/90 backdrop-blur-lg">
        <p id="container-window-info" class="text-center w-full h-full top-1/4 absolute text-white text-xl font-serif"></p>
      </div>
          ...
          <div class="basis-1/6 w-full h-full rounded-md">
            <div class="flex w-full h-full gap-1 justify-between">
              <button
                ...
                class="basis-1/2 p-2 rounded-full bg-red-800 text-white font-bold transition-all hover:bg-red-700 hover:scale-105"
              >
                Clash
              </button>
              <button
          ...
          <div class="basis-1/6 w-full h-full rounded-md">
            <div class="flex w-full h-full gap-1 justify-center">
              <button
                ...
                class="basis-1/2 border-2 border-gray-600 hover:bg-gray-700 text-gray-300 p-2 rounded-full flex justify-center items-center transition-all"
              >
                Close
              </button>
            </div>
          ...
          <div class="basis-1/6 w-full h-full rounded-md">
            <div class="flex w-full h-full gap-1 justify-between">
              <input
                ...
                class="basis-11/12 w-full h-full px-6 rounded-md bg-neutral-800 border border-gray-600 text-white focus:outline-none focus:border-red-500 focus:ring-1 focus:ring-red-500"
              />
              <button
                onclick="registerDomain()"
                class="p-2 rounded-full bg-red-700 text-white transition-all hover:bg-red-600"
              >
    ...
    <footer>
      <div class="fixed bottom-3 right-3 flex flex-col gap-1 z-50">
        <a href="${DONATE_LINK}" target="_blank">
          <button class="bg-red-600 text-white rounded-full border-2 border-neutral-800 p-1 block transition-all hover:bg-red-500 hover:scale-110">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="size-6">
              <path
        ...
        <button onclick="toggleWildcardsWindow()" class="bg-gray-700 text-white rounded-full border-2 border-neutral-800 p-1 transition-all hover:bg-gray-600 hover:scale-110 PLACEHOLDER_API_READY">
          <svg
        ...
        <button onclick="toggleDarkMode()" class="bg-gray-700 text-white rounded-full border-2 border-neutral-800 p-1 transition-all hover:bg-gray-600 hover:scale-110">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
            viewBox="0 0 24 24"
        ...
        const res = fetch(url).then(async (res) => {
          ...
          if (res.status == 200) {
            ...
            for (const domain of respJson) {
              ...
              // Diubah
              domainElement.classList.add("w-full", "bg-neutral-800", "border", "border-gray-700", "p-2", "rounded-md", "text-gray-300");
          ...
          new Promise(async (resolve) => {
            const res = await fetch("https://${serviceName}.${rootDomain}/check?target=" + target)
              .then(async (res) => {
                if (isActive) return;
                if (res.status == 200) {
                  pingElement.classList.remove("dark:text-white", "animate-pulse"); // Diubah
                  const jsonResp = await res.json();
                  if (jsonResp.proxyip === true) {
                    isActive = true;
                    pingElement.textContent = "Active " + jsonResp.delay + " ms " + "(" + jsonResp.colo + ")";
                    pingElement.classList.add("text-green-500"); // Diubah
                  } else {
                  ...
                  pingElement.classList.add("text-red-400"); // Diubah
                }
              })
              .finally(() => {
                resolve(0);
...
class Document {
  ...
  buildProxyGroup() {
    ...
    for (let i = 0; i < this.proxies.length; i++) {
      ...
      // Assign proxies
      // SEMUA STYLE KARTU DIUBAH DI SINI
      proxyGroupElement += `<div class="lozad scale-95 mb-2 bg-neutral-900 card-pulse transition-all duration-300 rounded-xl p-4 w-60 border border-red-900/50 shadow-lg shadow-red-900/20 hover:shadow-red-500/30 hover:border-red-700/80">`;
      // Ganti border di ikon bendera
      proxyGroupElement += `  <div id="countryFlag" class="absolute -translate-y-9 -translate-x-2 border-2 border-black rounded-full overflow-hidden"><img width="32" src="https://hatscripts.github.io/circle-flags/flags/${proxyData.country.toLowerCase()}.svg" /></div>`;
      proxyGroupElement += `  <div>`;
      proxyGroupElement += `    <div id="ping-${i}" class="animate-pulse text-xs font-semibold text-gray-400">Idle ${proxyData.proxyIP}:${proxyData.proxyPort}</div>`;
      proxyGroupElement += `  </div>`;
      // Ganti header
      proxyGroupElement += `  <div class="rounded py-1 px-2 bg-red-900/50 border-y border-red-800/50 mt-1">`;
      proxyGroupElement += `    <h5 class="font-bold text-lg text-red-400 mb-1 overflow-x-scroll scrollbar-hide text-nowrap">${proxyData.org}</h5>`;
      proxyGroupElement += `    <div class="text-gray-300 text-sm">`;
      ...
      for (let x = 0; x < proxyData.list.length; x++) {
        ...
        // STYLE TOMBOL DIUBAH DI SINI
        proxyGroupElement += `<button class="bg-gray-800 text-gray-300 text-xs rounded-md p-1.5 w-full transition-all duration-200 ease-in-out transform hover:bg-red-700 hover:text-white hover:scale-105 hover:shadow-lg hover:shadow-red-600/30" onclick="copyToClipboard('${proxy}')">${indexName[x]}</button>`;
  ...
  buildCountryFlag() {
    ...
    for (const flag of new Set(flagList)) {
      ...
      }" class="py-1 transition-all opacity-70 hover:opacity-100 hover:scale-125" ><img width=20 src="https://hatscripts.github.io/circle-flags/flags/${flag.toLowerCase()}.svg" /></a>`;
    }
  ...
  addPageButton(text, link, isDisabled) {
    // STYLE TOMBOL PAGE DIUBAH DI SINI
    const pageButton = `<li><button ${
      isDisabled ? "disabled" : ""
    } class="px-3 py-1 bg-red-700 border-2 border-red-500 text-white font-bold rounded transition-all hover:bg-red-600 hover:shadow-lg hover:shadow-red-500/30 disabled:bg-neutral-700 disabled:text-gray-500 disabled:border-neutral-600 disabled:shadow-none" onclick=navigateTo('${link}')>${text}</button></li>`;
