...
const CONFIG = {
  ...
  proxyPerPage: 24, // Jumlah server per halaman
...
// =========================================
// FRONTEND GENERATOR (PERBAIKAN FILTER)
// =========================================
class Document {
  constructor(request) {
    this.html = baseHTML;
  ...
  buildProxyGroup() {
    ...
    this.proxies.forEach((proxy, i) => {
      ...
      // Simpan Country Code asli di data attribute untuk filter
      cards += `
...
// =========================================
// HTML TEMPLATE (FIXED REGION MAP)
// =========================================
const baseHTML = `
<!DOCTYPE html>
...
<body class="antialiased min-h-screen flex flex-col">
    ...
    <header class="pt-28 pb-12 px-4 text-center relative overflow-hidden">
        ...
        <p class="text-gray-400 mb-8 max-w-2xl mx-auto text-sm md:text-base">
            Akses internet aman dan cepat dengan server premium kami.
        </p>
        ...
        <div class="max-w-xl mx-auto relative group">
            ...
            <div class="relative flex bg-[#1e293b] rounded-xl p-1.5">
                ...
                <input type="text" id="searchInput" class="w-full bg-transparent border-none text-white placeholder-gray-500 focus:ring-0 sm:text-sm px-3 py-2 focus:outline-none" placeholder="Cari Server (ex: ID, SG)...">
            </div>
        </div>
        ...
        <div class="flex flex-wrap justify-center gap-2 mt-6">
            ...
            <button onclick="filterRegion('Asia')" class="filter-btn px-4 py-1.5 rounded-full bg-white/5 hover:bg-white/10 text-gray-300 text-xs font-bold border border-white/5">Asia (SG/ID)</button>
    ...
    <script>
        ...
        function filterRegion(region) {
            ...
            if(region === 'all') {
                ...
                // MAP REGION LENGKAP (KODE & NAMA)
                const map = {
                    'Asia': ['ID', 'SG', 'JP', 'MY', 'KR', 'HK', 'CN', 'TW', 'TH', 'VN', 'Indonesia', 'Singapore', 'Japan', 'Malaysia'],
                    'Europe': ['DE', 'GB', 'NL', 'FR', 'RU', 'Germany', 'United Kingdom', 'Netherlands'],
                    'Americas': ['US', 'CA', 'BR', 'United States']
                };

                document.querySelectorAll('.search-item').forEach(item => {
                    const c = item.getAttribute('data-country');
                    const allowed = map[region] || [];
                    // Cek apakah salah satu kode/nama ada di dalam data-country (case insensitive)
                    const match = allowed.some(key => c.toLowerCase().includes(key.toLowerCase()));
