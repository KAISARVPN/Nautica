...
// Kelas untuk mengelola WebSocket
...
// Kelas untuk mendeteksi protokol
class ProtocolDetector {
  static async detect(buffer) {
    ...
    return helpers.reverse("ss"); // default
  }
}

// Kelas untuk parsing protokol
class ProtocolParser {
  static parseShadowsocks(ssBuffer) {
    ...
    switch (addressType) {
      ...
      default:
        return {
          hasError: true,
          message: `addressType tidak valid untuk ${helpers.reverse("ss")}: ${addressType}`,
        };
    }
    ...
    if (!addressValue) {
      return {
        hasError: true,
        message: `Alamat tujuan kosong, tipe alamat adalah: ${addressType}`,
      };
    }
  ...
  static parseVless(buffer) {
    ...
    switch (addressType) {
      ...
      default:
        return {
          hasError: true,
          message: `addressType tidak valid adalah ${addressType}`,
        };
    }
    ...
    if (!addressValue) {
      return {
        hasError: true,
        message: `addressValue kosong, addressType adalah ${addressType}`,
      };
    }
  ...
  static parseTrojan(buffer) {
    ...
    if (!addressValue) {
      return {
        hasError: true,
        message: `alamat kosong, addressType adalah ${addressType}`,
      };
    }
...
// Kelas untuk mengelola reverse proxy
...
// Kelas untuk mengelola konfigurasi
class ConfigManager {
  static generateAllConfig(request, hostName, proxyList, page = 0) {
    ...
    try {
      ...
      document.setTitle("Selamat Datang di <span class='text-blue-500 font-semibold'>Nautica</span>");
      ...
      for (let i = startIndex; i < startIndex + CONFIG.proxyPerPage; i++) {
        ...
        // Generate configs for each port and protocol
      ...
      return `Terjadi kesalahan saat membuat konfigurasi ${helpers.reverse("vless")}. ${error}`;
    }
  }
    ...
    for (const proxy of proxyList) {
      ...
      for (const port of filterPort) {
        for (const protocol of filterVPN) {
          ...
          uri.hash = `${result.length + 1} ${helpers.getFlagEmoji(proxy.country)} ${proxy.org} WS ${
            port == 443 ? "TLS" : "NTLS"
          } [${CONFIG.serviceName}]`;
    ...
    switch (filterFormat) {
      ...
      case "bfr":
        try {
          ...
          helpers.log("Error converting config:", error);
          return new Response("Konversi gagal", {
            status: 500,
            headers: {
              ...CORS_HEADERS,
            },
...
class CloudflareApi {
  ...
  async getDomainList() {
    try {
      ...
      helpers.log("Error getting domain list:", error);
      return [];
    }
  }
  ...
  async registerDomain(domain) {
    try {
      ...
      helpers.log("Error registering domain:", error);
      return 500;
    }
  }
}

// Kelas untuk membuat dokumen HTML
class Document {
  constructor(request) {
    this.html = baseHTML;
  ...
  buildProxyGroup() {
    ...
    for (let i = 0; i < this.proxies.length; i++) {
      ...
      proxyGroupElement += `    <div id="ping-${i}" class="animate-pulse text-xs font-semibold dark:text-white">Mengidentifikasi ${proxyData.proxyIP}:${proxyData.proxyPort}</div>`;
      proxyGroupElement += `  </div>`;
      ...
      for (let x = 0; x < proxyData.list.length; x++) {
        const indexName = [
          `${helpers.reverse("TROJAN")} TLS`,
          `${helpers.reverse("VLESS")} TLS`,
          `${helpers.reverse("SS")} TLS`,
          `${helpers.reverse("TROJAN")} NTLS`,
          `${helpers.reverse("VLESS")} NTLS`,
          `${helpers.reverse("SS")} NTLS`,
        ];
...
// HTML template - Tidak diubah sesuai permintaan
let baseHTML = `
<!DOCTYPE html>
<html lang="id" id="html" class="scroll-auto scrollbar-hide dark">
  <head>
    <meta charset="UTF-8" />
  ...
  <body class="bg-white dark:bg-neutral-800 bg-fixed">
    ...
    <footer>
      <div class="fixed bottom-3 right-3 flex flex-col gap-1 z-50">
        <a href="${CONFIG.urls.donate}" target="_blank">
    ...
    <script>
      ...
      async function copyToClipboardAsTarget(target) {
        windowInfoContainer.innerText = "M生成 konfigurasi...";
        const url = "${CONFIG.urls.converter}";
        const res = await fetch(url, {
          method: "POST",
          body: JSON.stringify({
            url: rawConfig,
      ...
      function toggleOutputWindow() {
        windowInfoContainer.innerText = "Pilih output:";
        toggleWindow();
      ...
      function checkProxy() {
        for (let i = 0; ; i++) {
          ...
          new Promise(async (resolve) => {
            const res = await fetch("https://${CONFIG.serviceName}.${CONFIG.rootDomain}/check?target=" + target)
              .then(async (res) => {
                if (isActive) return;
                if (res.status == 200) {
                  ...
                  pingElement.textContent = "Pemeriksaan Gagal!";
                }
              })
              .finally(() => {
                resolve(0);
    ...
    try {
      ...
      if (url.pathname.startsWith("/api/v1")) {
        ...
        if (apiPath.startsWith("/domains")) {
          if (!isApiReady) {
            return new Response("API tidak siap", {
              status: 500,
            });
          }
        ...
        // Handle subscription API
        if (apiPath.startsWith("/sub")) {
          return await ConfigManager.generateSubscription(request);
        }
